...
import { cancelEvent, EventDto, getEvents } from '../api/events';
...
export default function ActiveEvents() {
  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        if (isOnline) {
          const data = await getEvents({ status: 'future', type: selectedType || undefined, includeAdmin: 0, limit });
          const normalizedData = (data || []).map(normalizeEvent).filter(e => e && e.id);
          setEvents(normalizedData);
          setHasMore(normalizedData.length === limit);
          saveToOffline(normalizedData);
        } else {
          const offlineData = getOfflineData();
          setEvents(offlineData.map(normalizeEvent).filter(e => e && e.id));
        }
      } catch {
        const offlineData = getOfflineData();
        setEvents(offlineData.length > 0 ? offlineData.map(normalizeEvent).filter(e => e && e.id) : []);
      } finally {
        setLoading(false);
      }
    }
    ...
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h2 className="text-sm font-semibold text-app-text">Aktif Etkinlikler</h2>
            <p className="text-xs text-app-muted">
              Tarihi henüz geçmemiş (gelecek tarihli) etkinliklerin listesi.
        ...
        <div className="flex items-center gap-2">
          <div className={`flex items-center gap-1.5 rounded-lg px-3 py-2 text-[11px] font-semibold ${
            ...
            {isOnline ? <><Wifi className="h-3.5 w-3.5" /><span>Çevrimiçi</span></> : <><WifiOff className="h-3.5 w-3.5" /><span>Çevrimdışı</span></>}
          </div>
          {lastSync && !isOnline && (
            <div className="text-[11px] text-app-muted">Son sync: {new Date(lastSync).toLocaleTimeString('tr-TR')}</div>
          )}
        </div>
        ...
        <div className="flex items-center gap-2">
          <button
            type="button"
            onClick={() => exportEventsToExcel('aktif-etkinlikler.xlsx', exportRows)}
          ...
          <EventPdfDownloadLink filename="aktif-etkinlikler.pdf" title="Aktif Etkinlikler" rows={exportRows}>
            <button
              type="button"
              disabled={loading || exportRows.length === 0}
              ...
              title={exportRows.length === 0 ? 'Veri bulunamadı' : `PDF export (${exportRows.length} etkinlik)`}
            >
      ...
      <div id="active-events-table" className="overflow-hidden rounded-2xl border border-app-border bg-app-card shadow-sm backdrop-blur-[10px] transition">
        ...
        {loading ? (
          ...
          <div className="px-4 py-6 text-xs text-slate-400">Aktif etkinlik bulunmuyor.</div>
        ) : (
                ...
                return (
                  ...
                  >
                    ...
                    <div className="min-w-0">
                      <div className="truncate font-medium text-app-text">{ev?.title || 'Başlıksız'}</div>
                    </div>
                    <div className="text-[11px] text-app-muted">
                      {ev?.date ? new Date(ev.date).toLocaleString('tr-TR') : 'Tarih belirtilmemiş'}
                    </div>
